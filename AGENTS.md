...
- `tui/`: Go / Bubble Tea host TUI; `model.go` owns views and keybindings, `client.go` owns HTTP search, and `table.go` owns columns and rendering. Both `ctrl+q` and `ctrl+c` quit — the alt-screen/raw-mode terminal disables SIGINT/SIGTSTP entirely, so any other key (including a bare `ctrl+c` fallback) would otherwise do nothing and leave the process (and the recursive `make run` waiting on it) stuck in the foreground.
