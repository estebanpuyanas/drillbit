...
`make containers` (and therefore bare `make`) always runs `podman-compose up -d --build --force-recreate`, so it picks up current source on every run — Podman's build cache keeps a no-op rebuild cheap (layers unaffected by the diff are reused), and `--force-recreate` guarantees the running container is always recreated from whatever image is current, since this stack's podman-compose version (1.6.0) does not reliably recreate an already-running container from a freshly built/retagged image on its own. Never call raw `podman-compose up -d` directly to bring the stack up: it reuses whatever image already exists in the local Podman store with no rebuild attempt and no warning, so code changes since the last build silently never run. `make rebuild` bypasses the cache entirely for a guaranteed from-scratch build and also passes `--force-recreate` on its final `podman-compose up -d`. Both targets prune only the specific image IDs they just superseded (`scripts/prune-stale-images.sh`, scoped to `drillbit_backend`/`drillbit_mcp-server`/`drillbit_ramalama`) — never a system-wide `podman image prune`, since this host may run other unrelated container projects.
...
- `tui/`: Go / Bubble Tea host TUI; `model.go` owns views and keybindings, `client.go` owns HTTP search, `table.go` owns columns and rendering, and `history.go` owns the persisted search-history log (`$XDG_STATE_HOME/drillbit/history.json`, falling back to `~/.local/state`). Both `ctrl+q` and `ctrl+c` quit — the alt-screen/raw-mode terminal disables SIGINT/SIGTSTP entirely, so any other key (including a bare `ctrl+c` fallback) would otherwise do nothing and leave the process (and the recursive `make run` waiting on it) stuck in the foreground. `↑`/`↓` in the search input recall history entries (bubbles' `textinput` binds those keys to suggestion-cycling by default, but the app never sets suggestions, so repurposing them for history recall doesn't conflict). TUI tests must isolate `XDG_STATE_HOME` per test (see `testModel(t)` in `model_test.go`) so they never read or write the real user's history file.
- `backend/main.py`: FastAPI `/search` endpoint: vector search, BM25, RRF, COPR enrichment, LLM re-ranking. `/search` has three return paths (LLM re-ranked happy path, raw-candidate fallback when re-ranking fails, LLM-suggestion-only fallback when ChromaDB is empty) — all three must build results via `to_package_result()` so every path yields the same `PackageResult` field set; never construct a result dict by hand in a new path. `rerank_with_llm` asks the model for plain `package-name: reason` lines, not a JSON array of objects: measured live against RamaLama's llama3.2:3b, that JSON shape got flattened into a bare array of strings on nearly every call (an identical-prompt retry just reproduced the same wrong shape), while the line format — which mirrors how candidates are already listed in the prompt — was reliable. Also confirmed live: RamaLama's OpenAI-compatible endpoint silently ignores `response_format: {"type": "json_schema", ...}` (no grammar constraint is actually applied), so don't rely on it to force structured output from this model/runtime.
